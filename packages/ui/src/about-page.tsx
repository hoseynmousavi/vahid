...
function Icon({
  ...
  name: "spark" | "pin" | "heart" | "arrow" | "check";
}) {
...
function EditorialCollage({ content }: { content: AboutContent["artwork"] }) {
  return (
    <div
      className="relative mx-auto aspect-[4/5] w-full max-w-[570px]"
      aria-label="A visual story of travel, place, and"
      role="img"
    >
      <div className="absolute inset-x-[7%] bottom-[4%] top-[7%] overflow-hidden rounded-[2.5rem] bg-[#d9e6de] shadow-[0_35px_80px_-35px_rgba(23,53,43,0.45)]">
        ...
        <div className="absolute left-[12%] top-[14%] max-w-[13rem] text-[#17352b]">
          ...
          <p className="mt-3 text-3xl font-black leading-[0.95] tracking-[-0.05em] sm:text-4xl">
            {content.headline}
          </p>
        </div>
    ...
    <main className="min-h-screen overflow-hidden bg-[#fffaf1] text-[#17352b] selection:bg-[#f6c453] selection:text-[#17352b]">
      <header className="relative z-20 border-b border-[#17352b]/10">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 sm:px-8">
          ...
          >
            <SharedLink className="transition hover:text-[#df765c]" href="/">
              {content.navigation.findStay}
            </SharedLink>
            <span className="text-[#df765c]">{content.navigation.story}</span>
            <a className="transition hover:text-[#df765c]" href="#values">
              {content.navigation.values}
            </a>
          </nav>
          <SharedLink
            className="group inline-flex items-center gap-2 rounded-full bg-[#17352b] px-5 py-3 text-sm font-bold text-white transition hover:bg-[#244c3f]"
            href="/"
          >
        ...
        <div className="mx-auto grid min-h-[720px] max-w-7xl items-center gap-10 px-5 py-14 sm:px-8 lg:grid-cols-[0.92fr_1.08fr] lg:py-20">
          <div className="relative z-10">
            <div className="inline-flex items-center gap-2 rounded-full border border-[#17352b]/15 bg-white/60 px-4 py-2 text-xs font-bold uppercase tracking-[0.2em]">
              <span className="h-2 w-2 rounded-full bg-[#df765c]" />{" "}
            ...
            <h1 className="mt-7 max-w-[700px] text-5xl font-black leading-[0.92] tracking-[-0.06em] sm:text-7xl lg:text-[5.4rem]">
              ...
              <span className="relative inline-block text-[#df765c]">
            ...
            <div className="mt-9 flex flex-wrap items-center gap-5">
              <a
                className="group inline-flex items-center gap-3 rounded-full bg-[#df765c] px-6 py-3.5 text-sm font-bold text-white transition hover:bg-[#c95f48]"
                href="#story"
              >
              ...
              <p className="flex items-center gap-2 text-sm font-semibold">
                <span className="grid h-7 w-7 place-items-center rounded-full bg-[#dce9df]">
                  <Icon name="check" />
                </span>
                {content.hero.independentLabel}
              </p>
            </div>
          </div>
        ...
        <div className="mx-auto grid max-w-7xl divide-y divide-white/15 px-5 sm:grid-cols-3 sm:divide-x sm:divide-y-0 sm:px-8">
          {content.stats.map((stat) => (
            <div className="py-10 text-center sm:py-12" key={stat.label}>
              <p className="text-3xl font-black tracking-[-0.04em] text-[#f6c453] sm:text-4xl">
                {stat.value}
              </p>
      ...
      >
        <div className="grid gap-14 lg:grid-cols-[0.8fr_1.2fr] lg:gap-24">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.24em] text-[#df765c]">
          ...
          <div className="space-y-7 text-lg leading-8 text-[#486259]">
            ...
            <div className="flex items-center gap-4 pt-3 text-[#17352b]">
              <div className="grid h-12 w-12 place-items-center rounded-full bg-[#f6c453] font-black">
                {content.story.founderInitials}
              </div>
              <div>
                <p className="font-bold">{content.story.founders}</p>
                <p className="text-sm text-[#71837d]">
                  {content.story.founderDescription}
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="bg-[#edf1e9] py-24 sm:py-28" id="values">
        <div className="mx-auto max-w-7xl px-5 sm:px-8">
          <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
            ...
            <p className="max-w-md leading-7 text-[#5d716a]">
          ...
          <div className="mt-14 grid gap-5 md:grid-cols-3">
            {content.values.items.map((value, index) => (
              <article
                className="group relative min-h-[330px] overflow-hidden rounded-[2rem] border border-[#17352b]/10 bg-[#fffaf1] p-8 transition duration-300 hover:-translate-y-1 hover:shadow-xl"
                key={value.number}
              >
                <span className="absolute right-6 top-4 text-7xl font-black tracking-[-0.08em] text-[#17352b]/[0.045]">
                  {value.number}
                </span>
                <span className="grid h-12 w-12 place-items-center rounded-2xl bg-[#f6c453] text-[#17352b]">
                  <Icon name={valueIcons[index] ?? "spark"} />
                </span>
        ...
        <div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:gap-24">
          ...
          <ol className="relative border-l border-[#17352b]/20">
            {content.journey.milestones.map((milestone, index) => (
              ...
              >
                <span
                  className={`absolute -left-[7px] top-1 h-[13px] w-[13px] rounded-full border-[3px] border-[#fffaf1] ${index === content.journey.milestones.length - 1 ? "bg-[#df765c]" : "bg-[#17352b]"}`}
                />
                <p className="text-sm font-black text-[#df765c]">
                  {milestone.year}
                </p>
      ...
      <section className="px-5 pb-5 sm:px-8 sm:pb-8">
        <div className="relative mx-auto max-w-7xl overflow-hidden rounded-[2.5rem] bg-[#f6c453] px-6 py-20 text-center sm:px-12 sm:py-24">
          ...
          <div className="relative mx-auto max-w-3xl">
            ...
            <p className="mx-auto mt-6 max-w-xl text-lg leading-8 text-[#3c554d]">
              {content.callToAction.description}
            </p>
            <SharedLink
              className="group mt-9 inline-flex items-center gap-3 rounded-full bg-[#17352b] px-7 py-4 text-sm font-bold text-white transition hover:bg-[#244c3f]"
              href="/"
            >
      ...
      <footer className="mx-auto flex max-w-7xl flex-col gap-6 px-5 py-10 text-sm text-[#64766f] sm:flex-row sm:items-center sm:justify-between sm:px-8">
        <div className="flex items-center gap-3 text-[#17352b]">
          <Mark />
          <span className="font-black">{content.brand}</span>
        </div>
        <p>{content.footer.note}</p>
        <div className="flex gap-5 font-semibold text-[#17352b]">
          <a className="hover:text-[#df765c]" href="#story">
            {content.footer.story}
          </a>
          <a className="hover:text-[#df765c]" href="#values">
            {content.footer.values}
          </a>
          <SharedLink className="hover:text-[#df765c]" href="/">
            {content.footer.hotels}
          </SharedLink>
        </div>
      </footer>
    </main>
